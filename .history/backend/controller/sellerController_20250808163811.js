...
async function getSellerApplicationStatus(req, res) {
    try {
        const userId = req.userId;

        const user = await userModel.findById(userId).select('sellerStatus businessType sellerApplicationDate verificationDocuments phone address');
        if (!user) {
            return res.status(404).json({
                message: "User not found",
                error: true,
        ...
        res.json({
            ...
            data: {
                ...
                sellerApplicationDate: user.sellerApplicationDate || null,
                hasRequiredDocuments: user.verificationDocuments && user.verificationDocuments.length > 0,
                hasPhone: !!user.phone,
                hasCompleteAddress: !!(user.address && user.address.street && user.address.city && user.address.state)
            }
        });

    } catch (err) {
        console.error('Error in getSellerApplicationStatus:', err);
        res.status(500).json({
            message: err.message || "Internal server error",
            error: true,
            success: false

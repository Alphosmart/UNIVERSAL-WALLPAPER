            ...
            {menuDisplay && (
              <div className='absolute bg-white bottom-0 top-11 h-fit p-2 shadow-lg rounded'>
                <nav>
                  ...
                  {user?.role === 'ADMIN' && (
                    <Link to={"/admin-panel/all-users"} className='whitespace-nowrap hover:bg-slate-100 p-2' onClick={closeMenu}>
                      Admin Panel
                    </Link>
                  )}
                  
                  {/* Temporary admin access for testing */}
                  <Link to={"/admin-panel/dashboard"} className='whitespace-nowrap hover:bg-slate-100 p-2 text-red-600' onClick={closeMenu}>

    ...
    const handleApplyToBeSeller = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Enhanced error handling for missing profile information
                if (data.missingFields && data.missingFields.length > 0) {
                    ...
                    toast.error(
                        `Please complete your profile first:\n\n${requirements.join('\n')}\n\n➡️ Go to Profile → Edit Profile to add missing information`,
                        { autoClose: 10000 }
                    );
                } else {
                    ...
                    {sellerStatus.sellerStatus === 'not_seller' && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                                <h3 className="text-lg font-semibold text-yellow-800 mb-2">📋 Requirements Before Applying:</h3>
                                <div className="space-y-2 text-sm">
                                    <div className="flex items-center gap-2">
                                        <span className="text-yellow-600">📱</span>
                                        <span>Add your phone number in Profile settings</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-yellow-600">🏠</span>
                                        <span>Complete your address (street, city, state) in Profile</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-yellow-600">🆔</span>
                                        <span>Upload identification document (government ID or business license)</span>
                                    </div>
                                    
                                    {/* Quick ID Upload */}
                                    <div className="mt-4 p-3 bg-gray-50 rounded border border-gray-200">
                                        <label className="block text-sm font-medium text-gray-700 mb-2">
                                            📤 Quick ID Upload (Upload here to satisfy requirement):
                                        </label>
                                        <input
                                            type="file"
                                            accept="image/*,.pdf"
                                            onChange={async (e) => {
                                                ...
                                                if (file) {
                                                    try {
                                                        ...
                                                        // For now, just convert to base64 and save
                                                        const reader = new FileReader();
                                                        reader.onloadend = async () => {
                                                            try {
                                                                const response = await fetch(SummaryApi.uploadVerificationDocumentProfile.url, {
                                                                ...
                                                                if (data.success) {
                                                                    ...
                                                                    fetchSellerStatus(); // Refresh to check requirements
                                                                } else {
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            Accepted: JPG, PNG, PDF (Max 5MB). This will be saved to your profile.
                                        </p>
                                    </div>
                                    <div className="mt-3 p-2 bg-blue-50 rounded border-l-4 border-blue-400">
                                        <span className="text-blue-700 text-xs">
                                            💡 <strong>Tip:</strong> Go to Profile → Edit Profile to add missing information
                                        </span>
                                    </div>
                                </div>
                            </div>

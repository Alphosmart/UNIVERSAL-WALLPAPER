...
const BecomeSellerPage = () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.missingFields && data.missingFields.length > 0) {
                    toast.error(
                        <div>
                            <div className="font-bold">Complete your profile first:</div>
                            <div className="text-sm mt-1">
                                {data.missingFields.includes('phone number') && <div>• Add your phone number in Profile</div>}
                                {data.missingFields.includes('complete address (street, city, state)') && <div>• Add your complete address in Profile</div>}
                                {data.missingFields.includes('identification document (identity proof or business license)') && <div>• Upload identification document</div>}
                                <div className="mt-2 text-blue-600">Go to Profile → Edit Profile to add missing information</div>
                            </div>
                        </div>,
                        { autoClose: 8000 }
                    );
            ...
            console.error('Error applying to be seller:', error);
    ...
    const handleUpdateProfile = async () => {
        ...
        try {
            const response = await fetch(SummaryApi.updateProfileForSeller.url, {
                method: SummaryApi.updateProfileForSeller.method,
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (data.success) {
                toast.success(data.message);
                checkEligibility(); // Refresh eligibility after update
            } else {
                toast.error(data.message);
            }
        } catch (error) {
                ...
                try {
                    const response = await fetch(SummaryApi.uploadVerificationDocument.url, {
                        ...
                        body: JSON.stringify({
                            ...
                            documentUrl: reader.result // In production, this would be a URL from file storage
                        })
                    });
